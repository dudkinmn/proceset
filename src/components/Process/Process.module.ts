const getHHMM = (msec: string): string => {
  ...
  const hh =
    d.getHours() && d.getDay() == 0
      ? ""
      : `${(d.getDate() - 1) * 24 + d.getHours()} ч `;
...
const getPercent = (percentFrom: string, persentWhat: string): string => {
  ...
  const mAAT =
    ((dAAT.getDate() - 1) * 24 + dAAT.getHours()) * 60 + dAAT.getMinutes();
  const mALT =
    ((dALT.getDate() - 1) * 24 + dALT.getHours()) * 60 + dALT.getMinutes();
...
const getNumberWithText = (num: number, world: string): string => {
  ...
  if (
    ...
    numStr.substring(numStr.length - 2) !== "11"
  ) {
    textResult = textForms[0];
  } else if (
    /^[2-4]{1}$/.test(numStr[numStr.length - 1]) &&
    (num !== 12 || 13 || 14)
  ) {
...
export { getHHMM, getPercent, getDDMMYYYY, getNumberWithText };
