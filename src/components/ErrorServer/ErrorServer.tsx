...
export interface IErrorServerProps {
  errorText?: string;
}

type ErrorServerProps = IErrorServerProps;

let ErrorServer = ({ errorText }: ErrorServerProps): ReactElement => {
  const getServerErrorText = (error?: string): string => {
    let errorTextRu = "";

    switch (error) {
      case "GraphQL error: No user with that email":
        errorTextRu = "Неверный логин";
        break;
      case "GraphQL error: Incorrect password":
        errorTextRu = "Неверный пароль";
        break;
      case "Network error: Failed to fetch":
        errorTextRu = "Нет связи с сервером";
        break;
      default:
        errorTextRu = "Ошибка сервера";
        break;
    }
